...
const Profile: React.FC = () => {
  ...
  const handleFollowClick = async () => {
    ...
    try {
      ...
      const followerUsername = followerResponse.username;
      const followingUsername = profileId;

      if (isFollowing) {
        await axios.delete("/api/users/unfollow", {
          headers: { 'Content-Type': 'application/json' },
          data: { followerUsername, followingUsername },
        });
      } else {
        await axios.post("/api/users/follow", { followerUsername, followingUsername });
        handleClick();
      }
